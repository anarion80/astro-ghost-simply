export const getContrastRatio = (color1, color2) => {
    ...
    const contrastRatio = (luminance(hexToRgb(color1)) + 0.05) / (luminance(hexToRgb(color2)) + 0.05);
...
export const reduceBrightness = (hexColor, reduction) => {
    ...
    const rgb = hexToRgb(hexColor).map(c => Math.max(0, Math.round(c * reduction)));

    // Convert RGB back to hex
    const reducedHexColor = '#' + rgb.map(c => c.toString(16).padStart(2, '0')).join('');
